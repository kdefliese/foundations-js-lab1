...
// number of times the new caretaker fed the lion. one array entry per day
  var mealsPerDay = [ 5, 4, 3, 6, 2, 4, 3, 4, 5, 1 ],
    tooHungryDay, averageMealsPerDay, day, numerator;
  day = 0;
  averageMealsPerDay = 4;
  numerator = 0;
  while (averageMealsPerDay >= 4) {
    numerator += mealsPerDay[day];
    averageMealsPerDay = numerator / (day + 1);
    day++;
    tooHungryDay = day;
/* I assumed this should come out as mealsPerDay[0]= "day 1", not "day 0".
If the count should start from 0, then the last two lines should be reversed in order. */
  }
  return tooHungryDay;
};
